      ...
      <table class="table table-report -mt-2">
        <thead>
          <tr>
            ...
            <th v-if="selectable" class="text-center whitespace-nowrap">
              {{ $t("readersList.actions") }}
            </th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="reader in readers" v-bind:key="reader.id" class="intro-x">
            <td>
              <p class="font-medium whitespace-nowrap" @click="copyReaderId(reader)">
                {{ reader.name }}
              </p>
              <div
                class="text-gray-600 text-xs whitespace-nowrap mt-0.5"
                @click="copyReaderId(reader)"
              >
                {{ reader.id }}
              </div>
            </td>
            ...
            <td class="w-40">
              <div class="flex items-center justify-evenly">
                ...
                <Tippy
                  :content="
                    reader.card ? 'Reader has card inserted' : 'Reader has no card inserted'
                  "
                >
                  <CreditCardIcon
                    class="w-4 h-4 mr-2"
            ...
            <td class="table-report__action w-56" v-if="selectable">
              <div class="flex justify-center items-center">
                <div
                  ...
                  v-bind:class="{ selectDisabled: !reader.card }"
                >
